$(document).ready(function () {
          ...
          tx2.broadcast(function (data) {
            if ($(data).find("result").text() == "1") {
              ...
              $("#walletSendConfirmStatus")
                ...
                .html(
                  unescape($(data).find("response").text()).replace(/\+/g, " ")
                );
  ...
  $("#walletSpendTo .addressAdd").click(function () {
    ...
    $("#walletSpendTo .glyphicon-plus:last")
      .removeClass("glyphicon-plus")
      .addClass("glyphicon-minus");
    $("#walletSpendTo .glyphicon-minus:last")
      .parent()
  ...
  $("#newKeysBtn").click(function () {
    ...
    if (
      ...
      $("#aes256pass").val() == $("#aes256pass_confirm").val()
    ) {
    ...
    $("#newPrivKeyEnc").val(
      CryptoJS.AES.encrypt(coin.wif, $("#aes256pass").val()) + ""
    );
  });
  ...
  $("#newPaperwalletBtn").click(function () {
    ...
    paperwallet.document.write(
      '<h2>BTC PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddress"></div><p>' +
        $("#newBitcoinAddress").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Public Key</h3></div><div style="text-align: center;"><div id="qrpubkey"></div><p>' +
        $("#newPubKey").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Private Key (KEEP SECRET!)</h3></div><div style="text-align: center;"><div id="qrprivkey"></div><p>' +
        $("#newPrivKey").val() +
        "</p></div></div>"
    );
  ...
  $("#newSegwitPaperwalletBtn").click(function () {
    ...
    var paperwallet = window.open();
    paperwallet.document.write(
      '<h2>BTC SegWit PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddress"></div><p>' +
        $("#newSegWitAddress").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Public Key</h3></div><div style="text-align: center;"><div id="qrpubkey"></div><p>' +
        $("#newSegWitPubKey").val() +
        '</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Redeem Script</h3></div><div style="text-align: center;"><div id="qrredeem"></div><p>' +
  ...
  $("#newMultiSigAddress").click(function () {
    ...
    if (
      $("#multisigPubKeys .pubkey").parent().hasClass("has-error") == false &&
  ...
  $("#multisigPubKeys .pubkeyAdd").click(function () {
    if ($("#multisigPubKeys .pubkeyRemove").length < 14) {
      var clone =
        '<div class="form-horizontal">' + $(this).parent().html() + "</div>";
  ...
  $("#newTimeLockedAddress").click(function () {
    ...
    if (
      $("#timeLockedPubKey").parent().hasClass("has-error") == false &&
      $("#timeLockedDateTimePicker").parent().hasClass("has-error") == false
    ) {
      try {
        var hodl = coinjs.simpleHodlAddress(
    ...
    tx.listUnspent(redeem.addr, function (data) {
      if (redeem.addr) {
        $("#redeemFromAddress")
          .removeClass("hidden")
          .html(
            '<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="' +
              explorer_addr +
              redeem.addr +
              '" target="_blank">' +
              redeem.addr +
  ...
  function listUnspentBlockcypher(redeem, network) {
    $.ajax({
      ...
      success: function (data) {
        if (data.address) {
          // address field will always be present, txrefs is only present if there are UTXOs
          $("#redeemFromAddress")
            .removeClass("hidden")
            .html(
              '<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="' +
          ...
          for (var i in data.txrefs) {
            ...
            if (tx.match(/^[a-f0-9]+$/)) {
              var n = o.tx_output_n;
              var script =
                redeem.redeemscript == true ? redeem.decodedRs : o.script;
  ...
  function listUnspentCryptoidinfo_Carboncoin(redeem) {
    $.ajax({
      ...
      data:
        ...
        "&key=" +
        coinjs.key +
        "&setmodule=carboncoin&request=listunspent&address=" +
        redeem.addr,
      dataType: "xml",
      error: function () {
        $("#redeemFromStatus")
      ...
      success: function (data) {
        if ($(data).find("result").text() == 1) {
          ...
          $.each($(data).find("unspent").children(), function (i, o) {
            ...
            var amount = (($(o).find("value").text() * 1) / 100000000).toFixed(
              8
            );
            addOutput(tx, n, script, amount);
          });
  ...
  function listUnspentChainso(redeem, network) {
    $.ajax({
      type: "GET",
      url:
        "https://chain.so/api/v2/get_tx_unspent/" + network + "/" + redeem.addr,
      dataType: "json",
      error: function (data) {
        $("#redeemFromStatus")
          .removeClass("hidden")
    ...
    $.ajax({
      ...
      error: function (data) {
        $("#rawTransactionStatus")
          ...
          .removeClass("alert-success")
          .removeClass("hidden")
          .html(" There was an error submitting your request, please try again")
          .prepend(
            '<span class="glyphicon glyphicon-exclamation-sign"></span>'
          );
      },
      success: function (data) {
        $("#rawTransactionStatus")
          .html(unescape($(data).find("response").text()).replace(/\+/g, " "))
          .removeClass("hidden");
        if ($(data).find("result").text() == 1) {
          $("#rawTransactionStatus")
            .addClass("alert-success")
  ...
  function rawSubmitcryptoid_Carboncoin(thisbtn) {
    ...
    $.ajax({
      ...
      url:
        ...
        "&key=" +
        coinjs.key +
        "&setmodule=carboncoin&request=sendrawtransaction",
      data: { rawtx: $("#rawTransaction").val() },
      dataType: "xml",
      error: function (data) {
        $("#rawTransactionStatus")
        ...
        if ($(data).find("result").text() == 1) {
          $("#rawTransactionStatus")
            ...
            .html(
              " TXID: " +
                $(data).find("txid").text() +
                '<br> <a href="https://chainz.cryptoid.info/carbon/tx.dws?' +
                $(data).find("txid").text() +
                '" target="_blank">View on Blockchain Explorer</a>'
            );
        } else {
          $("#rawTransactionStatus")
            .addClass("alert-danger")
      ...
      success: function (data) {
        if (data.status && data.data.txid) {
          ...
          $("#rawTransactionStatus")
            ...
            .removeClass("alert-success")
            .removeClass("hidden")
            .html(" Unexpected error, please try again")
            .prepend(
              '<span class="glyphicon glyphicon-exclamation-sign"></span>'
            );
        }
  ...
  function rawSubmitblockcypher(thisbtn, network) {
    ...
    $.ajax({
      ...
      error: function (data) {
        var r =
          "Failed to broadcast: error code=" +
        ...
        if (data.tx && data.tx.hash) {
          $("#rawTransactionStatus")
            ...
            .html(
              " TXID: " +
                data.tx.hash +
                '<br> <a href="https://live.blockcypher.com/' +
                network +
                "/tx/" +
                data.tx.hash +
                '" target="_blank">View on Blockchain Explorer</a>'
          ...
          $("#rawTransactionStatus")
            ...
            .html(
              " TXID: " +
                ...
                '<br> <a href="https://blockchair.com/' +
                network +
                "/transaction/" +
                data.data.transaction_hash +
                '" target="_blank">View on Blockchain Explorer</a>'
    ...
    if (decode) {
      ...
      if (decode.type == "multisig__") {
        ...
        $("#verifyRsDataHodl .pubkey").val(
          coinjs.pubkey2address(decode["pubkey"])
        );
  ...
  function decodeHDaddress() {
    ...
    try {
      ...
      var hex_cmp_prv = Crypto.util.bytesToHex(
        coinjs.numToBytes(coinjs.hdkey.prv, 4).reverse()
      );
      var hex_cmp_pub = Crypto.util.bytesToHex(
        coinjs.numToBytes(coinjs.hdkey.pub, 4).reverse()
      );
  ...
  function deriveHDaddress() {
    ...
    for (var i = index_start; i <= index_end; i++) {
      if ($("#hdpathtype option:selected").val() == "simple") {
        ...
        var derived = hd.derive_path(
          $("#hdpath input").val().replace(/\/+$/, "") + "/" + i
        );
      }
      html += "<tr>";
      html += "<td>" + i + "</td>";
    ...
    if (dataArray) {
      for (var x in dataArray) {
        if (dataArray[x] && typeof dataArray[x] == "string") {
          if (
            dataArray[x].split("=")[0].toLowerCase().replace(/\[\]$/gi, "") ==
            value.toLowerCase()
          ) {
  ...
  $("#coinjs_coin").change(function () {
    ...
    if (o[5] == "false") {
      $("#coinjs_broadcast, #rawTransaction, #rawSubmitBtn, #openBtn").attr(
        "disabled",
        true
      );
    ...
    if (o[6] == "false") {
      $(
        "#coinjs_utxo, #redeemFrom, #redeemFromBtn, #openBtn, .qrcodeScanner"
      ).attr("disabled", true);
  ...
  $("#fees .slider").on("input", function () {
    ...
    $("." + $(this).attr("rel") + " .estimate", $("#fees")).removeClass(
      "hidden"
    );
  });
  ...
  $("#fees .txo_p2pkh").on("input", function () {
    var outputno = $(
      "." + $(this).attr("rel") + " .outputno",
      $("#fees .txoutputs")
    ).html();
    $("." + $(this).attr("rel") + " .bytes", $("#fees .txoutputs")).html(
      outputno * $("#est_txo_p2pkh").val() + outputno * 9
    );
    mathFees();
  });
  ...
  $("#fees .txo_p2sh").on("input", function () {
    ...
    $("." + $(this).attr("rel") + " .bytes", $("#fees .txoutputs")).html(
      outputno * $("#est_txo_p2sh").val() + outputno * 9
    );
    mathFees();
  });
  ...
  $("#fees .txi_regular").on("input", function () {
    var inputno = $(
      "." + $(this).attr("rel") + " .inputno",
      $("#fees .txinputs")
    ).html();
    $("." + $(this).attr("rel") + " .bytes", $("#fees .txinputs")).html(
      inputno * $("#est_txi_regular").val() + inputno * 41
    );
    mathFees();
  });
  ...
  $("#fees .txi_multisig").on("input", function () {
    ...
    $("." + $(this).attr("rel") + " .bytes", $("#fees .txinputs")).html(
      inputno * $("#est_txi_multisig").val() + inputno * 41
    );
    mathFees();
  });
  ...
  $("#fees .txi_hodl").on("input", function () {
    ...
    $("." + $(this).attr("rel") + " .bytes", $("#fees .txinputs")).html(
      inputno * $("#est_txi_hodl").val() + inputno * 41
    );
    mathFees();
  });
  ...
  $("#fees .txi_unknown").on("input", function () {
    ...
    $("." + $(this).attr("rel") + " .bytes", $("#fees .txinputs")).html(
      inputno * $("#est_txi_unknown").val() + inputno * 41
    );
    mathFees();
  });
  ...
  $("#feesAnalyseBtn").click(function () {
    ...
    for (var i = 0; i < deserialized.ins.length; i++) {
      ...
      if (script.type == "segwit") {
        ...
        $("#fees .segwit .inputno").html(
          $("#fees .segwit .inputno").html() * 1 + 1
        );
        $("#fees .txi_segwit").val($("#fees .txi_segwit").val() * 1 + 1);
        $("#fees .segwit .bytes").html(
          $("#fees .segwit .bytes").html() * 1 + size
        );
        ...
        $("#fees .multisig .inputno").html(
          $("#fees .multisig .inputno").html() * 1 + 1
        );
        $("#fees .txi_multisig").val($("#fees .txi_multisig").val() * 1 + 1);
        $("#fees .multisig .bytes").html(
          $("#fees .multisig .bytes").html() * 1 + size
        );
      } else if (script.type == "hodl") {
        size += 78;
        $("#fees .hodl .inputno").html(
          $("#fees .hodl .inputno").html() * 1 + 1
        );
        ...
        $("#fees .regular .inputno").html(
          $("#fees .regular .inputno").html() * 1 + 1
        );
        $("#fees .txi_regular").val($("#fees .txi_regular").val() * 1 + 1);
        $("#fees .regular .bytes").html(
          $("#fees .regular .bytes").html() * 1 + size
        );
      } else if (script.type == "unknown") {
        size += script.script.length / 2;
        $("#fees .unknown .inputno").html(
          $("#fees .unknown .inputno").html() * 1 + 1
        );
        $("#fees .txi_unknown").val($("#fees .txi_unknown").val() * 1 + 1);
        $("#fees .unknown .bytes").html(
          $("#fees .unknown .bytes").html() * 1 + size
        );
      }
    }
    ...
    for (var i = 0; i < deserialized.outs.length; i++) {
      if (deserialized.outs[i].script.buffer[0] == 118) {
        $("#fees .txoutputs .p2pkh .outputno").html(
          $("#fees .txoutputs .p2pkh .outputno").html() * 1 + 1
        );
        $("#fees .txoutputs .p2pkh .bytes").html(
          $("#fees .txoutputs .p2pkh .bytes").html() * 1 + 34
        );
        $("#fees .txo_p2pkh").val($("#fees .txo_p2pkh").val() * 1 + 1);
      } else if (deserialized.outs[i].script.buffer[0] == 169) {
        $("#fees .txoutputs .p2sh .outputno").html(
          $("#fees .txoutputs .p2sh .outputno").html() * 1 + 1
        );
        $("#fees .txoutputs .p2sh .bytes").html(
          $("#fees .txoutputs .p2sh .bytes").html() * 1 + 32
        );
        $("#fees .txo_p2sh").val($("#fees .txo_p2sh").val() * 1 + 1);
      }
    }
  ...
  function feeStats() {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#fees .recommended .blockTime").html(
          $(data).find("timestamp").text()
        );
        $("#fees .recommended .blockDateTime").html(
          unescape($(data).find("datetime").text()).replace(/\+/g, " ")
        );
        $("#fees .recommended .txId").html(
          '<a href="https://coinb.in/tx/' +
            $(data).find("txid").text() +
            '" target="_blank">' +
            $(data).find("txid").text() +
            "</a>"
  ...
  function getMouseXY(e) {
    ...
    if (
      $("#entropybucket").html().indexOf(xEnt) == -1 &&
      $("#entropybucket").html().indexOf(yEnt) == -1
    ) {
